...
int main(int argc, char** argv) {
    ...
    /* zero out the array so it is fully initialised *
     was in the wrong place, and setting theb array to a constant state of 0
     It has been moved*/
    ...
    /* open the input file */
    /*number changed in order to accept the required specific argunment*/
    fpRead = gropen(argv[1]);
    /* load the data and handle errors */
    result = load(list, fpRead);
    /* check that the data loaded successfully */
    /*number changed so that incoming data could not cause an error*/
    if (result <= 0) {
