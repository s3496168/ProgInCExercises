...
struct load_data {
    /* the path to the file that the user enetered. This might be an absolute
     * ore a relative path. Please do not over-validate this as you should just
...
struct quit_data {
    ...
    BOOLEAN save;
    /**
     * What is the path we wish to save to? Clearly this should be NULL if we
     * are not saving.
     **/
    const char* path;
};
 ...
 * our system
 **/
void init_action_array(actionfunc[NUM_ACTIONS]);
