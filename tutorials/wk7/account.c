...
BOOLEAN is_owner(const struct account* acc, const char key[]) {
    return strcmp(acc->key, key) == 0;
}
...
BOOLEAN read_account(struct account* account) {
    ...
    strcpy(account->key, line);
    if (get_string("Please enter the customer balance", line, LINELEN) == RTM) {
        return FALSE;
    }
