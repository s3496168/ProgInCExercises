...
void cypher(char *s, int shift)
{
        ...
        while (s)
        {
                ...
                if (isalpha(*s))
                {
                        ...
                        {
                                if (ch > 'z'){
                                        /*while loops ensure that the function runs
                                        more than once*/
                                        while (ch > 'z')
                                        ch -= ALPHABETLEN;
                                }
 ...
 * not too long.
 ****************************************************************/
enum result get_input(const char *prompt, char *output, unsigned len)
{
        char *result;
...
enum result get_int(const char *prompt, int *output)
{
        ...
        /*addedd missing if statement to deal with non numeric data*/
        if(strlen(int_str)==0){
                fprint(stderr, "Error: input must be numeric.\n");
                return FAILURE;
        }
        switch (res)
        {
