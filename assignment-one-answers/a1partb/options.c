...
void reverse_string(char str[]) {
    normal_output("Hello from reverse a string.\n");
}
...
void guess_a_number(long seed) {
    normal_output("Hello from guess a number.\n");
}
...
struct falsible_register can_give_change(const cash_register reg,
    ...
    normal_output("Hello from can_give_change()\n");
    return result;
}
...
enum ttt_result ttt_check_win(ttt_board board) {
    normal_output("Hello from tictactoe.\n");
    return TTT_ONGOING;
}
